...
export default function Navbar() {
  ...
  useEffect(() => {
    if (trackingData && !isLoading && !error) {
      navigate("/track-shipment");
    }
  }, [trackingData, isLoading, error, navigate]);
  return (
    <nav className="bg-body-tertiary py-4 lg:py-2 border-b relative z-[99999]">
      <div className="navbar-container w-full flex items-center justify-between px-4 2xl:px-44 ">
        <span className="text-xl font-semibold">
          {i18n.language === "en" ? <SvgIconEn /> : <SvgIconAr />}
        </span>
        <div className="flex lg:hidden justify-center items-center gap-x-3 ">
          <div
            id="track-btn"
            onMouseEnter={() => {
              setIsHovered(true);
            ...
            className="nav-item track-btn text-mainColor flex justify-between items-baseline py-5 px-6 hover:shadow gap-1 relative"
          >
            <span className="nav-link text-base font-bold text-mainColor">
              {t("Navbar.trackShipment")}
            </span>
            <span className="w-4 h-4">
              <i
                className={`fa-solid ${
                  ((isHovered || isHovered2) && i18n.language) === "en"
                    ? "fa-angle-right"
                    : ((isHovered || isHovered2) && i18n.language) === "ar"
                    ? "fa-angle-left"
                    : "fa-angle-down"
                }`}
              ></i>
            ...
            {(isHovered || isHovered2) && (
              <div
                ...
                className={`track-input-container bg-white absolute border rounded-lg rounded-se-none w-[200%] end-0 top-full flex flex-col gap-y-4 z-[9999] p-6 py-8`}
              >
                <p className="text-textColor text-start">
                  {t("Navbar.trackYourShipment")}
                </p>
                <div className="track-input relative h-12 flex">
                  <input
                    value={inputValue}
                    onChange={(e) => {
                    ...
                    className="absolute top-0 h-full w-5/6 start-0 p-2 px-3 border rounded-s-lg border-textGray outline-[#1dacb480]"
                    type="number"
                    placeholder={t("Navbar.trackingNo")}
                  />
                  <span
                    onClick={!isLoading ? handleTrackingChange : undefined} // Disable onClick when loading
                    className={`search-btn ${
                      isLoading ? "disabled" : ""
                    } bg-secondMainColor search-btn w-1/6 text-white fs-3 absolute top-0  end-0 h-full flex justify-center items-center py-5 cursor-pointer rounded-e-lg`}
                  >
                    {isLoading ? (
        ...
        <div
          className={`lg:flex ${
            isOpen ? "flex" : "hidden"
          } top-full left-0 w-full lg:w-auto bg-body-tertiary lg:relative absolute bg-white lg:bg-transparent z-[999] h-screen lg:h-auto flex-col lg:flex-row gap-y-4`}
        >
          <ul className="navbar-nav mb-2 lg:mb-0 flex justify-start lg:justify-center items-start lg:items-center flex-col lg:flex-row w-full lg:h-auto px-4 lg:px-0 top-full bottom-0 end-0 start-0">
            <Link
              to="/"
              className="nav-item w-full py-6 lg:py-5 lg:px-6 border-b-2 border-textColor border-dotted lg:border-0"
            >
          ...
          <div className="flex justify-center items center lg:hidden">
            <button className="rounded-full px-5 w-2/3 border border-black py-3 font-bold">
              {t("Navbar.signIn")}
            </button>
          </div>
        </div>
        <div
          className={`lg:flex hidden top-full left-0 w-full  lg:w-auto bg-body-tertiary lg:relative absolute`}
        >
          <ul className="navbar-nav mb-2 lg:mb-0 flex justify-center items-center  flex-col lg:flex-row w-full  top-full bottom-0 end-0 start-0">
            <li
              id="track-btn"
              onMouseEnter={() => {
                setIsHovered(true);
              ...
              className="nav-item track-btn text-mainColor flex justify-between items-baseline py-5 px-6 hover:shadow gap-1"
            >
              <span className="nav-link text-base font-bold text-mainColor">
                {t("Navbar.trackShipment")}
              </span>
              ...
              {(isHovered || isHovered2) && (
                <div
                  ...
                  className={`track-input-container bg-white absolute border rounded-lg rounded-ss-none w-11/12 start-0 top-full flex flex-col gap-y-4 z-50 p-6 py-8`}
                >
                  <p className="text-textColor text-start">
                    {t("Navbar.trackYourShipment")}
                  </p>
            ...
            <li className="nav-item">
              <span className="nav-link py-5 px-6 text-base font-bold">
                {t("Navbar.signIn")}
              </span>
            </li>
